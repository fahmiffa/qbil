            ...
            <div class="p-4 sm:p-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <select wire:model.live="router_id" class="w-full sm:w-auto bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-colors font-semibold text-slate-700 dark:text-slate-300">
                            @foreach($routers as $router)
                            <option value="{{ $router->id }}">{{ $router->name }} ({{ $router->host }})</option>
                            @endforeach
                        </select>
                        <button wire:click="openCreate()" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold py-2 px-4 rounded-lg transition-all shadow-sm">
