<div>
    ...
    <div class="w-full">
        ...
        @if($syncIsolationCustomers->isNotEmpty())
        <div x-show="!$wire.isSyncAlertDismissed" class="mb-6 group relative">
            <div @click="$wire.set('showSyncIsolationModal', true)" class="cursor-pointer bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-transparent border border-amber-200 dark:border-amber-500/20 rounded-2xl p-4 flex items-center justify-between transition-all hover:shadow-lg hover:shadow-amber-500/10">
                <div class="flex items-center gap-4">
                    <div class="relative">
                        <div class="relative w-12 h-12 bg-amber-500 text-white rounded-full flex items-center justify-center shadow-lg shadow-amber-500/20">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div>
                        <h3 class="text-amber-700 dark:text-amber-400 font-black tracking-tight">SINKRONISASI ISOLIR (TUNGGAKAN INVOICE)</h3>
                        <p class="text-sm text-amber-600/90 dark:text-amber-200/90 font-medium">
                            Terdapat <span class="font-bold underline text-amber-800 dark:text-amber-100">{{ $syncIsolationCustomers->count() }} pelanggan</span> memiliki tagihan belum lunas dari bulan sebelumnya s/d bulan sekarang.
                        </p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <div class="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-md">
                        <span>Lihat Daftar</span>
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
                    </div>
                    <button @click.stop="$wire.set('isSyncAlertDismissed', true)" class="p-2 text-amber-400 hover:text-amber-600 dark:hover:text-amber-200 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
    ...
    <!-- Sync Isolation List Modal (Tunggakan Bulan Sebelumnya s/d Sekarang) -->
    <div x-data="{ show: @entangle('showSyncIsolationModal') }"
        x-show="show"
        x-cloak
        class="fixed inset-0 z-[60] overflow-y-auto"
        style="display: none;">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white dark:bg-slate-800 rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full border border-slate-200 dark:border-slate-700">
                <div class="px-8 py-6 border-b border-slate-50 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h3 class="text-xl font-black text-slate-800 dark:text-white tracking-tight">Sinkronisasi Isolir (Tunggakan Invoice)</h3>
                        <p class="text-sm text-slate-500 mt-1">Daftar pelanggan yang memiliki tagihan belum lunas dari bulan sebelumnya sampai saat ini</p>
                    </div>
                    <div class="flex items-center gap-3">
                        @if($syncIsolationCustomers->isNotEmpty())
                        <button type="button" @click="
                            Swal.fire({
                                title: 'Isolir Massal ke MikroTik?',
                                text: 'Aplikasi akan mengirim perintah isolir ke MikroTik untuk {{ $syncIsolationCustomers->count() }} pelanggan yang memiliki tunggakan tagihan.',
                                icon: 'warning',
                                showCancelButton: true,
                                confirmButtonColor: '#ef4444',
                                cancelButtonColor: '#64748b',
                                confirmButtonText: 'Ya, Isolir Semua!',
                                cancelButtonText: 'Batal',
                                background: document.documentElement.classList.contains('dark') ? '#1e293b' : '#ffffff',
                                color: document.documentElement.classList.contains('dark') ? '#f1f5f9' : '#0f172a'
                            }).then((result) => {
                                if (result.isConfirmed) {
                                    $wire.bulkIsolateCustomers()
                                }
                            })
                        " class="inline-flex items-center gap-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold py-2.5 px-4 rounded-xl transition-all shadow-sm">
                            <svg wire:loading.remove wire:target="bulkIsolateCustomers" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
                            </svg>
                            <svg wire:loading wire:target="bulkIsolateCustomers" class="animate-spin w-4 h-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                            <span>Isolir Massal ({{ $syncIsolationCustomers->count() }})</span>
                        </button>
                        @endif
                        <button @click="show = false" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-0 max-h-[60vh] overflow-y-auto">
                    <table class="min-w-full divide-y divide-slate-100 dark:divide-slate-700">
                        <thead class="bg-slate-50/50 dark:bg-slate-900/40 sticky top-0 z-10 backdrop-blur-md">
                            <tr>
                                <th class="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Pelanggan</th>
                                <th class="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Bulan Belum Terbayar</th>
                                <th class="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Tagihan</th>
                                <th class="px-6 py-4 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                            @forelse($syncIsolationCustomers as $c)
                            @php
                                $unpaidInvoices = $c->invoices->where('status', 'unpaid')->where('billing_period', '<=', now()->format('Y-m'));
                                $totalAmount = $unpaidInvoices->sum('total_amount');
                            @endphp
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors align-top">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <a href="{{ route('customers.detail', $c->id) }}" class="flex flex-col group transition-colors" wire:navigate>
                                        <span class="font-bold text-slate-900 dark:text-white group-hover:text-blue-600 transition-colors">{{ $c->name }}</span>
                                        <span class="text-[10px] text-slate-400 font-medium group-hover:text-blue-400 transition-colors">{{ $c->phone ?? '-' }}</span>
                                        <span class="text-[10px] text-slate-400 font-mono">{{ $c->username }} ({{ $c->package->name ?? '-' }})</span>
                                    </a>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-1.5 max-w-xs">
                                        @foreach($unpaidInvoices as $inv)
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold bg-rose-50 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300 border border-rose-200/60 dark:border-rose-700/40">
                                            {{ \Carbon\Carbon::parse($inv->billing_period)->translatedFormat('M Y') }}
                                        </span>
                                        @endforeach
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="font-bold text-rose-600 dark:text-rose-400">Rp {{ number_format($totalAmount, 0, ',', '.') }}</span>
                                    <span class="block text-[10px] text-slate-400 font-medium">{{ $unpaidInvoices->count() }} invoice</span>
                                </td>
                                <td class="px-6 py-4 text-right whitespace-nowrap">
                                    <div class="flex items-center justify-end gap-2">
                                        <button type="button" @click="
                                            Swal.fire({
                                                title: 'Isolir Pelanggan {{ $c->name }}?',
                                                text: 'Pelanggan ini akan diisolir di MikroTik karena memiliki tunggakan tagihan.',
                                                icon: 'warning',
                                                showCancelButton: true,
                                                confirmButtonColor: '#ef4444',
                                                cancelButtonColor: '#64748b',
                                                ...
                                                if (result.isConfirmed) {
                                                    $wire.isolateCustomer({{ $c->id }})
                                                }
                                            })
                                        " class="inline-flex items-center gap-1.5 bg-rose-600 hover:bg-rose-700 text-white text-[10px] font-black px-3.5 py-1.5 rounded-xl shadow-sm transition-all uppercase">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
                                            </svg>
                                            Isolir
                                        </button>
                                        @if($latestInvoice = $unpaidInvoices->last())
                                        <button wire:click="sendWhatsappNotification('{{ $latestInvoice->id }}')" class="inline-flex items-center gap-1.5 bg-emerald-500 hover:bg-emerald-600 text-white text-[10px] font-black px-3.5 py-1.5 rounded-xl shadow-sm transition-all uppercase">
                                            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.414 0 .018 5.394 0 12.03c0 2.119.552 4.188 1.598 6.049L0 24l6.104-1.602a11.834 11.834 0 005.937 1.57h.005c6.632 0 12.028-5.391 12.03-12.028a11.85 11.85 0 00-3.529-8.52" />
                                            </svg>
                                            Kirim WA
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center text-slate-400 dark:text-slate-500 italic">
                                    Tidak ada tunggakan invoice di bulan-bulan sebelumnya.
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="px-8 py-6 bg-slate-50 dark:bg-slate-900/30 text-center border-t border-slate-50 dark:border-slate-700">
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Gunakan menu Verifikasi pada tagihan terkait untuk mengubah status bayar pelanggan.</p>
                </div>
            </div>
        </div>
    </div>
