<div class="p-6 space-y-6">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Manajemen Voucher</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Daftar pesanan voucher dari portal publik</p>
        </div>
    </div>
                            ...
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-3">
                                    ...
                                    @if($order->hotspotUsers->count() > 0)
                                        <svg class="w-4 h-4 text-slate-300 transform transition-transform {{ $selectedOrderId == $order->id ? 'rotate-180' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M19 9l-7 7-7-7"></path></svg>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-5">
        ...
        window.addEventListener('swal:confirm', event => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Livewire.dispatch(data.callback, { orderId: data.id });
                }
            });
        });
    </script>
